...
```
User Request → Risk Assessment → Update Strategy → Validation → Commit
                    ↓
...
**Risk Tiers**:
- **LOW**: Patch versions (1.2.3 → 1.2.4), automated updates
- **MEDIUM**: Minor versions (1.2.x → 1.3.0), ESLint, Vitest, TypeScript
- **HIGH**: Major versions (1.x → 2.0), MCP SDK, Typesense, Node runtime
...
📋 CHANGES PLANNED:
├─ package.json
│  ├─ typescript: 5.3.3 → 5.4.2 (MEDIUM risk)
│  └─ vitest: 1.0.0 → 2.0.0 (MEDIUM risk)
...
✅ TO EXECUTE: Run same command without --dry-run
❌ TO CANCEL: No action needed (no changes made)
```
...
**Error Handling**:
- If operation invalid → Report error, suggest correct syntax
- If no changes needed → Report "Already up to date"
...
**Algorithm**:
1. Run `npm outdated` → Capture outdated packages
2. Run `npm audit` → Capture security vulnerabilities
3. Check Node version alignment:
   ...
   if [ "$nvmrc_version" != "$dockerfile_version" ] || [ "$nvmrc_version" != "$package_min" ]; then
     echo "⚠️ Node version mismatch detected"
   fi
   ```
...
5. Format output:
   ...
   🔴 OUTDATED (X packages):
   - package@current → latest (risk: LOW/MEDIUM/HIGH)
...
3. **Display changelog summary**:
   ```
   📋 CHANGELOG for <package> (<current> → <target>):
...
1. **Validate version format**:
   ```bash
   if ! [[ "$version" =~ ^[0-9]+$ ]]; then
     echo "❌ Version must be major version number (e.g., 20, 22)"
     exit 1
   fi
   ```
...
5. **Verify alignment**:
   ...
   if [ "$nvmrc" == "$version" ] && [ "$dockerfile" == "$version" ] && [ "$package" == "$version" ]; then
     echo "✅ Node version aligned to $version"
   else
...
3. **Test if health check tool exists in base image**:
   ...
   if [ $? -ne 0 ]; then
     echo "❌ HEALTHCHECK will fail at runtime"
     echo "Tool '$HEALTHCHECK_CMD' not found in $BASE_IMAGE"
   else
     echo "✅ Health check tool available"
   fi
   ```
...
5. **Report findings and required fix**:
   ```
   ⚠️ DOCKER HEALTHCHECK DEPENDENCY VERIFICATION
...
6. **Validate fix** (after manual Dockerfile edit):
   ...
   if [ $? -eq 0 ]; then
     echo "✅ Health check dependency verified"
   else
     echo "❌ Health check dependency still missing"
     exit 1
   fi
...
|-------------------|------|------|-------|------|----------------|-----------------|
| `*-alpine` | ❌ | ❌ | ❌ | ❌ | apk | `RUN apk add --no-cache curl bash` |
| `*-slim` | ✅* | ❌ | ❌ | ✅ | apt-get | Usually included, verify |
| `debian:*` | ✅ | ✅ | ✅ | ✅ | apt-get | Usually included |
| `ubuntu:*` | ✅ | ✅ | ✅ | ✅ | apt-get | Usually included |
| `centos:*` / `rhel:*` | ✅ | ✅ | ✅ | ✅ | yum/dnf | Usually included |
| `distroless/*` | ❌ | ❌ | ❌ | ❌ | N/A | Use exec form, not shell |
| `scratch` | ❌ | ❌ | ❌ | ❌ | N/A | Static binary health checks only |

*"Usually included" ≠ guaranteed. Always verify with `docker run --rm <image> which <tool>`*
...
**❌ Alpine without dependency install**:
```dockerfile
FROM node:22-alpine
WORKDIR /app
COPY . .
...
**❌ Multi-stage build installing in wrong stage**:
```dockerfile
# Build stage
FROM node:22-alpine AS builder
RUN apk add --no-cache curl  # ❌ WRONG STAGE
WORKDIR /app
COPY . .
RUN npm run build
...
# Production stage
FROM node:22-alpine
RUN apk add --no-cache curl  # ✅ Install in FINAL stage
WORKDIR /app
COPY --from=builder /app/dist ./dist
...
8. **Generate report**:
   ...
   ✅ Auto-fixed: X vulnerabilities
   ⚠️ Manual review needed: Y vulnerabilities
   ❌ No fix available: Z vulnerabilities
...
1. **Verify emergency status**:
   ...
   if [ $? -ne 0 ]; then
     echo "⚠️ No critical vulnerabilities detected. Use /deps update instead."
     exit 1
   fi
   ```
...
```bash
npm install || {
  echo "❌ npm install failed (network error?)"
...
**Common scenarios**:
- **Alpine**: Missing curl, wget, bash → Use `apk add --no-cache <tool>`
- **Debian slim**: Missing curl, wget → Use `apt-get install -y <tool>`
- **Distroless**: No package manager → Switch to base image with shell access or use TCP health checks
- **Multi-stage builds**: Tool installed in builder stage but not in final stage → Install in FINAL stage
...
## Appendix A — Dependency & Version Control Files to Check
...
### 1. package-lock.json
- **Reason**: Dependency upgrade (vitest 1.0.0 → 2.0.0)
- **Changes**: 47 packages updated
- **Tests**: ✅ All pass
- **Validated**: 2025-12-30

### 2. Dockerfile
- **Reason**: Node 20 → Node 22 alignment
- **Changes**: `FROM node:20-alpine` → `FROM node:22-alpine`
- **Tests**: ✅ Build + integration tests pass
- **Validated**: 2025-12-30
...
- Critical CVE (CVSS ≥ 9.0) in runtime or base image
